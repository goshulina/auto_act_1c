...
from google_spreadsheet_DB import google_spreadsheets
...
def get_current_pokupki():
    address = 'https://pass:login@terminal.scloud.ru/base/odata/standard.odata/Document_ПоступлениеТоваровУслуг'
...
def get_contragents_dogovors():
    address = 'https://pass:login@terminal.scloud.ru/base/odata/standard.odata/Catalog_ДоговорыКонтрагентов'
    try:
                ...
                if i in value:
                    ...
                    for d in all_records:
                        if d['Счет'] == i and str(d['Акт №']) not in acts:  # and count == 0:
                            ...
                            ### address = "https://pass:login@terminal.scloud.ru/base/odata/standard.odata/Document_ПоступлениеТоваровУслуг(guid'{}')/Post()".format(
                            ###     guid_key
                            ### )
